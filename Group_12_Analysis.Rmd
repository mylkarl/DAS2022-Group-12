...
The data set in this report has 7 groups of variables, which are animal_Type, month, year, intake_type, outcome_type, chip_status, and time_at_shelter. Each set of variables had 1135 observations. Time_at_shelter is selected as the dependent variable Y for the convenience of analysis. At the same time, the missing values in the observed values are removed in the calculation

The group needs to discuss which factors influence the number of days an animal spends in the shelter before deciding on its final outcome.

For this problem, the team chose the GLM Poisson regression model.
